  ...
  return (
    ...
    >
      ...
      <span
        ...
        {
            loadingExtras ? ( <p className="text-center">Cargando...</p> ) 
            : errorExtras ? ( <p className="text-center">Hubo un error</p> ) 
            : extras.length === 0 ? ( <p className="text-center">No hay extras</p> ) 
            : (
              <>
              {
                extras.map((extra) => (
                  <article key={extra.id} className="grid grid-cols-3 justify-center items-center">
                      <h3>
                          {extra.nombre}
                      </h3>
                      <span>
                      ...
                      <ButtonPri onClick = {() => handleAdd(extra)} nombre="Agregar" />
                  </article>
              ))
              }
              </>
